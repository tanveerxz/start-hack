...
# Baseline life support power draw every sol — computers, pumps, crew habitat,
# water processors. NOT tied to active control corrections.
# Normal generation (35-50 kWh) comfortably covers this.
# Dust storm generation (14-20 kWh) cannot, so battery depletes ~10-16 kWh/sol.
BASELINE_LIFE_SUPPORT_KWH = 30.0
...
def _mars_temp_pressure(day: int, mars_env: MarsEnvironment) -> float:
    ...
    """
    sol_angle     = (2 * math.pi * day) / 365
    diurnal_angle = (2 * math.pi * (day % 1))
    seasonal_offset = 2.0 * math.sin(sol_angle)
...
def _mars_humidity_drift(state: GreenhouseState) -> float:
    ...
    The greenhouse loses humidity through seal imperfections,
    partially offset by crew respiration and plant transpiration.
    """
    transpiration_gain = random.uniform(2.0, 5.0)
    seal_loss          = -random.uniform(3.0, 7.0)
    crew_gain          = 0.5 * 4
...
def _apply_co2_control(current: float, drift: float, power_available: float) -> tuple[float, float]:
    ...
    if raw > CO2_SAFETY_MAX_PPM:
        ...
        raw = CO2_SAFETY_MAX_PPM - 100

    CONTROL_EFFICIENCY = 0.70
    error        = TARGET_CO2_PPM - raw
    # CO2 is a human safety system — always runs at full power regardless of budget
    correction   = _clamp(error, -MAX_CO2_CORRECTION_PPM, MAX_CO2_CORRECTION_PPM) * CONTROL_EFFICIENCY
    power_used   = abs(correction) * 0.02
    sensor_noise = random.gauss(0, 5.0)
    return round(_clamp(raw + correction + sensor_noise, 400.0, 2000.0), 1), round(power_used, 2)
...
def _apply_led_control(natural_par: float, power_available: float) -> tuple[float, float]:
    ...
    power_factor  = min(power_available / 20.0, 1.0)
    supplement    = min(shortfall, MAX_PAR_CORRECTION) * power_factor
    power_used    = supplement * 0.05
...
def _check_failure_events(state: GreenhouseState, day: int) -> dict[str, bool]:
    ...
    """
    failures = {
        "led_malfunction":          random.random() < 0.005,
        "temp_control_failure":     random.random() < 0.003,
        "co2_imbalance":            random.random() < 0.004,
        "nutrient_contamination":   random.random() < 0.002,
        "water_recycling_decline":  random.random() < 0.008,
    }
...
def simulate_sol(
    ...
    Steps:
      1. Compute Mars-driven drifts
      2. Check for failure events
      3. Apply control systems within power budget
      4. Deduct baseline life support draw
      5. Regenerate power from solar panels
      6. Update and return new GreenhouseState
    """
    day = state.day + 1
    ...
    if failures.get("co2_imbalance"):
        co2_drift   += random.uniform(100, 300)
    ...
    # ── Step 4: Baseline life support draw ───────────────────────────────────
    # Computers, pumps, crew habitat, water processors — always-on load.
    # Normal generation (35-50 kWh) comfortably covers this.
    # Dust storm generation (14-20 kWh) cannot — battery depletes ~10-16 kWh/sol.
    power_remaining -= BASELINE_LIFE_SUPPORT_KWH

    # ── Step 5: Solar regeneration ────────────────────────────────────────────
    ...
    # ── Step 6: Build and return new state ───────────────────────────────────
    new_state = GreenhouseState(
        ...
        nutrient_n_ppm         = state.nutrient_n_ppm,
